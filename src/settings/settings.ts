...
function CreateAllFileConfig(): LatestSyncConfigVersion {
    return {
        ...SYNCER_CONFIG_SCHEMA_MANAGER.getDefault().unsafeUnwrap(),
        syncQuery: "*",
...
function CreateDefaultSyncConfig(): LatestSyncConfigVersion {
    return SYNCER_CONFIG_SCHEMA_MANAGER.getDefault().unsafeUnwrap();
}
        ...
        const resetList = () => {
            ...
            const createElement = (elem: LatestSyncConfigVersion) => {
                ...
                new Setting(liContainer).setName("Edit syncer filter query").addButton((cb) => {
                    cb.setIcon("pencil").onClick(() => {
                        void FileAccess.getAllFileNodes(this.app, CreateDefaultSyncConfig()).then(
                            (nodes) => {
                                if (nodes.err) {
                                    LogError(LOGGER, nodes.val);
                                    return;
                                }
                ...
                new Setting(liContainer).setName("Edit raw file query").addButton((cb) => {
                    cb.setIcon("pencil").onClick(() => {
                        void FileAccess.getAllFileNodes(this.app, CreateAllFileConfig()).then(
                            (nodes) => {
                                if (nodes.err) {
                                    LogError(LOGGER, nodes.val);
                                    return;
                ...
                cacheSize.innerText = `Cache size: calculating....`;
                new Setting(liContainer)
                    .setName("Clear firestore cache")
                    .setDesc("Clear the cache of firestore entries")
                    .addButton((cb) => {
                        cb.setIcon("eraser").onClick(() => {
                            void FirebaseCache.writeToFirebaseCache(this.app, elem, []).then(
                                (res) => {
                                    if (res.err) {
                                        LogError(LOGGER, res.val);
                                        return;
                                    }
                                    cacheSize.innerText = `Cache size: 0`;
                                }
                            );
                        });
                    });
                void FirebaseCache.readFirebaseCache(this.app, elem).then((res) => {
                    if (res.err) {
                        LogError(LOGGER, res.val);
                        return;
                    }
                    cacheSize.innerText = `Cache size: ${res.safeUnwrap().cache.length}`;
                });
