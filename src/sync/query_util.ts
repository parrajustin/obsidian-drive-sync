...
export function ShouldHaveFileId(filePath: FilePathType, config: LatestSyncConfigVersion): boolean {
...
export function IsObsidianFile(filePath: FilePathType, config: LatestSyncConfigVersion): boolean {
    const searchString = GetQueryString(config.obsidianFileSyncQuery);
    return ChecksPassedFilter(filePath, searchString);
}

/** Checks if the file path lead to a raw file on the device. */
export function IsLocalFileRaw(filePath: FilePathType, config: LatestSyncConfigVersion): boolean {
